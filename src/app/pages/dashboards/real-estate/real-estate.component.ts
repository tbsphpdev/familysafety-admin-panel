...
*/
  ...
  renderCharts(charts: any) {
    const el = document.querySelector(".apex-charts") as HTMLElement | null;
    if (!el) return;
    if (charts === '1') {
      this.chart = new ApexCharts(el, this.totalrevenueChart);
    } else if (charts === '2') {
      this.chart = new ApexCharts(el, this.totalincomeChart);
    } else if (charts === '3') {
      this.chart = new ApexCharts(el, this.propertysaleChart);
    } else {
      this.chart = new ApexCharts(el, this.propetryrentChart);
    }
  }
}
